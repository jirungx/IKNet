...
def run_experiment(
    ...
    windows = split_by_rolling_window(price_df, train_years, test_years, start_year, end_year)
            ...
            for train_start, test_start, train_df, test_df in windows:
                try:
                    ...
                    pd.DataFrame({"date": dates, "y_true": y_true, "y_pred": y_pred}).to_csv(
                        os.path.join(out_pred_dir, f"IKNet_{test_start}_k{num_keywords}.csv"), index=False
                    )
