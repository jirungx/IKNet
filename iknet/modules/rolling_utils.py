...
def normalize_and_sequence(train_df, test_df, feature_cols, time_steps, horizon):
    ...
    # 원본 데이터프레임을 보존하기 위해 명시적으로 복사본 사용
    train_df_original = train_df.copy()
    test_df_original = test_df.copy()

    # --- Target (close) Scaling: scaler_y ---
    # 1. scaler_y를 원본 train 'close' 값으로 학습시킵니다.
    scaler_y.fit(train_df_original[['close']])

    # 2. 원본 train/test 'close' 값을 scaler_y로 변환하여 target 컬럼 생성 (별도 저장)
    y_train_scaled = scaler_y.transform(train_df_original[['close']])
    y_test_scaled = scaler_y.transform(test_df_original[['close']])

    # --- Feature Scaling: scaler_x ---
    feature_cols_x = feature_cols.copy() # 원본 코드처럼 'close' 포함 가정

    # 3. scaler_x를 원본 train 피처 값으로 학습시킵니다.
    scaler_x.fit(train_df_original[feature_cols_x])

    # 4. 원본 train/test 피처 값을 scaler_x로 변환합니다. (이 결과가 모델의 X 입력으로 사용됨)
    ...
    # --- DataFrame 형태로 다시 만들기 (make_sliding_sequences 함수가 DataFrame 입력을 받는 경우) ---
    # scaled feature 값들로 DataFrame을 다시 만듭니다.
    ...
    # 날짜 정보와 스케일링된 target 값을 추가합니다.
    train_df_scaled['date'] = train_df_original['date']
    test_df_scaled['date'] = test_df_original['date']
    train_df_scaled['target_close'] = y_train_scaled # scaler_y로 스케일링된 target
    test_df_scaled['target_close'] = y_test_scaled  # scaler_y로 스케일링된 target

    # --- Create Sequences ---
    # 스케일링된 피처(X)와 스케일링된 타겟(y)을 사용하여 시퀀스 생성
    # X는 scaler_x로 스케일링된 피처들을 사용하고, y는 scaler_y로 스케일링된 'target_close'를 사용
